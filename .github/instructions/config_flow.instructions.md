...
3. **Subentry Flow** (`config_flow.py`):
...
5. `entry.runtime_data = OpenMoticsData(...)` → stores runtime objects (from `data.py`)
...
- Place config flow in `config_flow.py`
- Place options flow in `config_flow.py`
- Place subentry flow in `config_flow.py` (if needed)
- Place shared logic in `config_flow.py`
