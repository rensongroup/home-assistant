...
**File:** `custom_components/openmotics/[platform].py`

### 2. Platform `__init__.py` Template

```python
"""[Platform] platform for OpenMotics Home Assistant integration Beta."""
...
async def async_setup_entry(
    ...
    coordinator: OpenMoticsCloudDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]

    async_add_entities(
        [
            OpenMotics[EntityName](coordinator, entry),
...
```python
"""[Entity description] for OpenMotics Home Assistant integration Beta."""
...
Add platform to `custom_components/openmotics/manifest.json`:
...
class OpenMotics[EntityName](
    ...
    def __init__(
        ...
        """Initialize entity."""
        super().__init__(coordinator, index, device, [PlatformEntityClass] )
        self._attr_unique_id = f"{entry.entry_id}_[entity_key]"
...
def native_value(self) -> str | None:
        try:
            self._device = self.coordinator.data["<entity>"][self.index]
...
async def async_press(self) -> None:
    """Handle button press."""
    await self.coordinator.api_client.trigger_action(self.device_id,)
