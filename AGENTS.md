...
- `custom_components/openmotics/` - Main integration code
...
- Integration logs: `custom_components.openmotics: debug` in `config/configuration.yaml`
...
- Use the domain `openmotics` for all DOMAIN references
- Prefix all integration-specific classes with `OpenMotics`
- Use "OpenMotics Home Assistant integration Beta" as the display title
...
- Inherit from platform base + `OpenMoticsEntity`
...
- `tests/` mirrors `custom_components/openmotics/` structure
